...
TARGET_SOC=60
CHARGE_CURRENT_PER_SOC_DEFICIT_UA=10000
...
run_pi_controller() {
    ...
    local capacity=$1; local soc_deficit=$((TARGET_SOC - capacity)); local target_current_ua=0; if [ "$soc_deficit" -gt 0 ]; then target_current_ua=$(( -1 * soc_deficit * CHARGE_CURRENT_PER_SOC_DEFICIT_UA )); fi
    local actual_current_ua=$(sample_median); [ -z "$actual_current_ua" ] && actual_current_ua=0
    local error=$(( target_current_ua - actual_current_ua )); local p_term=$(echo "scale=6; $KP * $error" | bc -l); integral_error=$(echo "scale=6; $integral_error + ($error * $LOOP_SLEEP_S)" | bc -l)
    local max_integral=2000000; local min_integral=-2000000; integral_error=$(awk -v v="$integral_error" -v mn="$min_integral" -v mx="$max_integral" 'BEGIN{ if(v>mx) print mx; else if(v<mn) print mn; else print v }')
    local i_term=$(echo "scale=6; $KI * $integral_error" | bc -l); local adjustment=$(printf "%.0f" "$(echo "scale=6; $p_term + $i_term" | bc -l)")
    local current_input_limit=$(cat "$LIMIT_FILE" 2>/dev/null || echo 0); local new_input_limit=$(( current_input_limit - adjustment ))
    if [ "$new_input_limit" -gt "$MAX_INPUT_CURRENT_UA" ]; then new_input_limit=$MAX_INPUT_CURRENT_UA; fi; if [ "$new_input_limit" -lt "$MIN_INPUT_CURRENT_UA" ]; then new_input_limit=$MIN_INPUT_CURRENT_UA; fi
    local diff=$(( new_input_limit > current_input_limit ? new_input_limit - current_input_limit : current_input_limit - new_input_limit ))
    debug "SoC=${capacity}% TargetCurr=${target_current_ua}uA BatCurr=${actual_current_ua}uA Err=${error} Adj=${adjustment} NewLimit=${new_input_limit}"
    if [ "$diff" -ge "$MIN_STEP_UA" ]; then
        info "SoC=${capacity}% | Target: ${target_current_ua}uA | Actual: ${actual_current_ua}uA | New Limit: ${new_input_limit}uA"
        if ! printf "%d" "$new_input_limit" > "$LIMIT_FILE" 2>/dev/null; then echo "$new_input_limit" | sudo tee "$LIMIT_FILE" >/dev/null || warn "Failed to write new_input_limit."; fi
    else
        debug "Delta ${diff} < MIN_STEP_UA ${MIN_STEP_UA}; skipping write."
    fi
}
